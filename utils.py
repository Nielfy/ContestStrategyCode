...
def get_trader(request_type):
    channel = grpc.insecure_channel(TRADING_CHANNEL)
...
def new_order(request_type, order_id, side, symbol, volume, price, is_market, pos_type):
    ...
    if list(response.orders.orders)!=[]:
        return list(response.orders.orders)[0]

def cancel_order(order_id):
    channel = grpc.insecure_channel(TRADING_CHANNEL)
    stub = broker_pb2_grpc.BrokerStub(channel)
    response = stub.cancel_order(broker_pb2.TraderRequest(trader_id=TRADER_ID,
                                                          trader_pin=TRADER_PIN,
                                                          request_type=common_pb2.CANCEL_ORDER,
                                                          order_id=order_id))
    return response
