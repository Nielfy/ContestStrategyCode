...
def run(s):
    for instrument in instruments:
        if instrument.symbol == s:
            ...
            longbias = max(min(0.07 * (1.0 - p), 0.1), miniumbidinterval)
            shortbias = max(min(0.07 * p, 0.1), miniumaskinterval)
            flag1=0
            flag2=0
            ...
            #                                            common_pb2.LONG)
            if mktposition.unrealizedPnllong[instrument.symbol] < -3000 or mktposition.unrealizedPnllong[
                instrument.symbol] > 4000 or mktposition.occupiedcashlong[instrument.symbol] > 20000 or mktposition.marketLongPosition[instrument.symbol]>400:
                # utils.new_order(common_pb2.NEW_ORDER, None, common_pb2.ASK, instrument.symbol,
                #                 mktposition.marketLongPosition[instrument.symbol], None,
                #                 True, common_pb2.LONG)
                orders[instrument.symbol].insert_order(common_pb2.ASK, instrument.symbol,
            ...
            #                                            meanp[instrument.symbol] + shortbias, True,
            #                                            common_pb2.SHORT)
            if mktposition.unrealizedPnlshort[instrument.symbol] < -3000 or mktposition.unrealizedPnlshort[
                instrument.symbol] > 4000 or mktposition.occupiedcashshort[instrument.symbol] > 20000 or mktposition.marketShortPosition[instrument.symbol]>400:
                # utils.new_order(common_pb2.NEW_ORDER, None, common_pb2.BID, instrument.symbol,
                #                 mktposition.marketShortPosition[instrument.symbol], None,
                #                 True, common_pb2.SHORT)
                orders[instrument.symbol].insert_order(common_pb2.BID, instrument.symbol,
